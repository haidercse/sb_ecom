            ...
            <div class="card-body">
                <form class="form-horizontal" action="{{ url('admin/update-banner/'.$banner->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group row">
                        <label class="col-md-3 col-form-label">{{translate('Banner Slug')}}</label>
                        <div class="col-md-9">
                            <input type="text" placeholder="{{translate('Slug')}}" name="name" value="{{$banner->name}}" class="form-control" required>
                        </div>
                    </div>
